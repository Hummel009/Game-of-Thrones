				...
				if (!result.isEmpty()) {
					...
					for (GOTEntityCart cart : result) {
						if (cart.getPulling() == sender.ridingEntity) {
							...
							sender.getServerForPlayer().getEntityTracker().func_151247_a(cart, GOTPacketHandler.getNetworkWrapper().getPacketFrom(new GOTPacketCargocartUpdate(-1, cart.getEntityId())));
							return null;
						}
					...
					sender.getServerForPlayer().getEntityTracker().func_151247_a(closest, GOTPacketHandler.getNetworkWrapper().getPacketFrom(new GOTPacketCargocartUpdate(closest.getPulling().getEntityId(), closest.getEntityId())));
				}
			}
			return null;
		}
