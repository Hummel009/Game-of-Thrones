...
import got.common.world.structure.westeros.crownlands.GOTStructureCrownlandsBase;
import net.minecraft.item.ItemStack;
import net.minecraft.world.World;

public class GOTStructureTobhoMottSmithy extends GOTStructureCrownlandsBase {
	public GOTStructureTobhoMottSmithy(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		associateBlockAlias("DOOR", fenceGateBlock);
		...
		this.placeChest(world, random, 3, 1, 6, GOTRegistry.chestBasket, 5, GOTChestContents.WESTEROS);
		this.placeChest(world, random, 2, 5, -3, GOTRegistry.chestBasket, 5, GOTChestContents.WESTEROS);
		placePlateWithCertainty(world, random, -1, 6, 3, GOTRegistry.ceramicPlateBlock, GOTFoods.WESTEROS);
		this.placeMug(world, random, -2, 6, 3, 0, GOTFoods.WESTEROS_DRINK);
		placeWeaponRack(world, -3, 1, 8, 1, getRandomWeapon(random));
		placeWeaponRack(world, 3, 1, 8, 3, getRandomWeapon(random));
		spawnNPCAndSetHome(new GOTEntityTobhoMott(world), world, 0, 1, 6, 8);
		spawnNPCAndSetHome(new GOTEntityGendryBaratheon(world), world, 0, 1, 6, 8);
		return true;
	}

	public ItemStack getRandomWeapon(Random random) {
		ItemStack[] items = { new ItemStack(GOTRegistry.westerosSword), new ItemStack(GOTRegistry.westerosDagger), new ItemStack(GOTRegistry.westerosDaggerPoisoned), new ItemStack(GOTRegistry.westerosSpear), new ItemStack(GOTRegistry.westerosHammer) };
