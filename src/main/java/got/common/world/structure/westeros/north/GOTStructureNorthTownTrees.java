...
public class GOTStructureNorthTownTrees extends GOTStructureNorthBase {
	public GOTStructureNorthTownTrees(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (int i12 : new int[] { -4, 0, 4 }) {
			...
			j1 = 1;
			int k12 = 0;
			for (int l = 0; l < 16 && ((treeGen = GOTStructureNorthTownTrees.getRandomTree(random).create(notifyChanges, random)) == null || !treeGen.generate(world, random, getX(i12, k12), getY(j1), getZ(i12, k12))); ++l) {
			}
		}
		return true;
	}
