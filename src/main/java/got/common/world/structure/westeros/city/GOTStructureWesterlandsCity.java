...
import got.common.world.structure.westeros.market.GOTStructureWesterlandsMarketStall;
import got.common.world.structure.westeros.storage.GOTStructureWesterlands.*;
import net.minecraft.block.Block;
import net.minecraft.init.Blocks;
import net.minecraft.util.MathHelper;
import net.minecraft.world.World;

public class GOTStructureWesterlandsCity extends GOTVillageGen {
	public boolean isTown;
	public boolean isCastle;

	public GOTStructureWesterlandsCity(GOTBiome biome, float f) {
		super(biome);
		gridScale = 12;
	...
	public GOTStructureWesterlandsCity setIsCastle() {
		isCastle = true;
		return this;
	}

	public GOTStructureWesterlandsCity setIsTown() {
		isTown = true;
		return this;
	}
	...
	public class Instance extends GOTVillageGen.AbstractInstance {
		...
		public Instance(GOTStructureWesterlandsCity village, World world, int i, int k, Random random, LocationInfo loc) {
		...
		public GOTStructureBase getRandomFarm(Random random) {
			if (random.nextBoolean()) {
				...
				return new GOTStructureWesterlandsVillageFarmCrops(false);
			}
		...
		public void setupCastle(Random random) {
			...
			this.addStructure(new GOTStructureWesterlandsFortress(false), 0, 12, 2, true);
			...
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), -23, -33, 2, true);
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), 23, -33, 2, true);
			...
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), -33, -23, 1, true);
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), -33, 23, 1, true);
			...
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), -23, 33, 0, true);
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), 23, 33, 0, true);
			...
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), 33, -23, 3, true);
			this.addStructure(new GOTStructureWesterlandsWatchtower(false), 33, 23, 3, true);
			...
			this.addStructure(new GOTStructureWesterlandsStables(false), -24, 2, 0);
			this.addStructure(new GOTStructureWesterlandsStables(false), -24, -2, 2);
			this.addStructure(new GOTStructureWesterlandsSmithy(false), 24, 1, 0);
			this.addStructure(new GOTStructureWesterlandsSmithy(false), 24, -1, 2);
			this.addStructure(new GOTStructureWesterlandsStoneHouse(false), -3, -25, 1);
			this.addStructure(new GOTStructureWesterlandsStoneHouse(false), 3, -25, 3);
			this.addStructure(new GOTStructureWesterlandsVillageFarmCrops(false), -18, -21, 1);
			this.addStructure(new GOTStructureWesterlandsVillageFarmCrops(false), 18, -21, 3);
		...
		public void setupTown(Random random) {
			...
			this.addStructure(new GOTStructureNPCRespawner(false) {
				...
				@Override
				public void setupRespawner(GOTEntityNPCRespawner spawner) {
					spawner.setSpawnClass(GOTEntityWesterlandsMan.class);
			...
			for (int k1 = -1; k1 <= 1; ++k1) {
				...
				if (random.nextInt(3) != 0) {
					this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), -stallPos + 3, k2, 1, true);
				}
				if (random.nextInt(3) == 0) {
					continue;
				}
				this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), stallPos - 3, k2, 3, true);
			}
			if (random.nextInt(3) != 0) {
				this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), 0, stallPos - 3, 0, true);
			}
			if (random.nextInt(3) != 0) {
				this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), 0, -stallPos + 3, 2, true);
			}
			int flowerX = 12;
			int flowerZ = 18;
			...
			for (int k1 = -1; k1 <= 1; ++k1) {
				int houseZ = k1 * 12;
				if (k1 == 1) {
					this.addStructure(new GOTStructureWesterlandsStoneHouse(false), -houseX, houseZ, 1, true);
					this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseX, houseZ, 3, true);
				}
				if (k1 == 0) {
					continue;
				}
				this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseZ, houseX, 0, true);
				this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseZ, -houseX, 2, true);
			}
			this.addStructure(new GOTStructureWesterlandsSmithy(false), 0, -26, 2, true);
			this.addStructure(new GOTStructureWesterosObelisk(false), 0, 27, 0, true);
			this.addStructure(new GOTStructureWesterlandsTavern(false), -houseX, -5, 1, true);
			...
			this.addStructure(new GOTStructureWesterlandsBath(false), houseX + 2, -6, 3, true);
			...
			for (int k1 = -4; k1 <= 4; ++k1) {
				...
				if (treepiece) {
					this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), -houseX - 2, houseZ, 1, true);
					this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), houseX + 2, houseZ, 3, true);
				} else {
					this.addStructure(new GOTStructureWesterlandsStoneHouse(false), -houseX, houseZ, 1, true);
					this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseX, houseZ, 3, true);
				}
				if (treepiece) {
					this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), houseZ, -houseX - 2, 2, true);
				} else {
					this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseZ, -houseX, 2, true);
				}
				if (Math.abs(k1) >= 2 && (!outerTavern || k1 <= 2)) {
					if (treepiece) {
						this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), houseZ, houseX + 2, 0, true);
					} else {
						this.addStructure(new GOTStructureWesterlandsStoneHouse(false), houseZ, houseX, 0, true);
					}
				}
			...
			if (outerTavern) {
				this.addStructure(new GOTStructureWesterlandsTavern(false), 44, houseX, 0, true);
			}
			int gardenX = 42;
			int gardenZ = 48;
			this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), -gardenX, -gardenZ, 1, true);
			this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), -gardenX, gardenZ, 1, true);
			this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), gardenX, -gardenZ, 3, true);
			this.addStructure(new GOTStructureWesterlandsVillageFarm.Tree(false), gardenX, gardenZ, 3, true);
			int obeliskX = 62;
			int obeliskZ = 66;
			...
			this.addStructure(new GOTStructureWesterlandsGatehouse(false), 0, 84, 2, true);
			this.addStructure(new GOTStructureWesterosLampPost(false), -4, 73, 0, true);
			this.addStructure(new GOTStructureWesterosLampPost(false), 4, 73, 0, true);
			int towerX = 78;
			int towerZ = 74;
			for (int i1 : new int[] { -towerX, towerX }) {
				this.addStructure(new GOTStructureWesterlandsWatchtower(false), i1, -towerZ, 2, true);
				this.addStructure(new GOTStructureWesterlandsWatchtower(false), i1, towerZ, 0, true);
			}
			int wallZ = 82;
			int wallEndX = 76;
			for (l = 0; l <= 3; ++l) {
		...
		public void setupVillage(Random random) {
			...
			this.addStructure(new GOTStructureWesterlandsCottage(false), -21, 0, 1);
			this.addStructure(new GOTStructureWesterlandsCottage(false), 0, -21, 2);
			this.addStructure(new GOTStructureWesterlandsCottage(false), 21, 0, 3);
			this.addStructure(new GOTStructureWesterlandsTavern(false), 0, 21, 0);
			if (random.nextBoolean()) {
				if (random.nextInt(3) == 0) {
					this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), -9, -12, 1);
				}
				if (random.nextInt(3) == 0) {
					this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), 9, -12, 3);
				}
				if (random.nextInt(3) == 0) {
					this.addStructure(GOTStructureWesterlandsMarketStall.getRandomStall(random, false), -9, 12, 1);
				}
