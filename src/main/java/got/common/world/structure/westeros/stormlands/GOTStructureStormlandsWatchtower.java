...
public class GOTStructureStormlandsWatchtower extends GOTStructureStormlandsBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		setBlockAndMetadata(world, 3, 6, 0, GOTRegistry.brick1, 5);
		placeWallBanner(world, 0, 5, -3, GOTItemBanner.BannerType.RENLY, 2);
		for (j1 = 1; j1 <= 9; j1++) {
		...
		placeBanner(world, 0, 14, 0, GOTItemBanner.BannerType.RENLY, 2);
		...
		GOTEntityStormlandsSoldier levyman = new GOTEntityStormlandsSoldier(world);
		spawnNPCAndSetHome(levyman, world, 0, 11, 1, 16);
		GOTEntityNPCRespawner respawner = new GOTEntityNPCRespawner(world);
		respawner.setSpawnClass(GOTEntityStormlandsSoldier.class);
