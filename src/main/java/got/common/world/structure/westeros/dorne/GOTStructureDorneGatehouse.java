...
public class GOTStructureDorneGatehouse extends GOTStructureDorneBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (int k14 : new int[] { -3, 3 }) {
			...
			for (int i16 = -2; i16 <= 2; ++i16) {
				...
				if (i23 == 0) {
					setBlockAndMetadata(world, i16, 10, k14, GOTRegistry.brick3, 8);
					continue;
				}
		...
		setBlockAndMetadata(world, -2, 10, 2, Blocks.torch, 2);
		setBlockAndMetadata(world, 2, 10, 2, Blocks.torch, 1);
		placeWallBanner(world, 1, 10, -3, GOTItemBanner.BannerType.MARTELL, 0);
		placeWallBanner(world, -1, 10, -3, GOTItemBanner.BannerType.MARTELL, 0);
		setBlockAndMetadata(world, -3, 12, -3, Blocks.torch, 3);
		setBlockAndMetadata(world, 3, 12, -3, Blocks.torch, 3);
		setBlockAndMetadata(world, -3, 12, 3, Blocks.torch, 4);
		setBlockAndMetadata(world, 3, 12, 3, Blocks.torch, 4);
		placeWallBanner(world, -3, 7, -3, GOTItemBanner.BannerType.MARTELL, 2);
		placeWallBanner(world, 3, 7, -3, GOTItemBanner.BannerType.MARTELL, 2);
		placeWallBanner(world, 3, 7, 3, GOTItemBanner.BannerType.MARTELL, 0);
		placeWallBanner(world, -3, 7, 3, GOTItemBanner.BannerType.MARTELL, 0);
		for (i1 = -5; i1 <= 5; ++i1) {
			i22 = Math.abs(i1);
			if (i22 >= 4) {
		...
		for (i1 = -9; i1 <= 9; ++i1) {
			i22 = Math.abs(i1);
			if (i22 == 5 || i22 == 8) {
				setBlockAndMetadata(world, i1, 3, 1, GOTRegistry.brick3, 8);
				continue;
			}
			if (i22 < 4) {
				continue;
