...
public abstract class GOTStructureWesterlandsVillageFarm extends GOTStructureWesterlandsBase {
	...
	public static class Animals extends GOTStructureWesterlandsVillageFarm {
		...
		public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
			...
			for (int l = 0; l < animals; ++l) {
				EntityAnimal animal = GOTStructureWesterlandsBarn.getRandomAnimal(world, random);
				int i12 = 3 * (random.nextBoolean() ? 1 : -1);
				int k12 = 3 * (random.nextBoolean() ? 1 : -1);
				spawnNPCAndSetHome(animal, world, i12, 1, k12, 0);
				animal.detachHome();
			}
			return true;
		}
	...
	public static class Crops extends GOTStructureWesterlandsVillageFarm {
		...
		public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
			...
			for (int l = 0; l < farmhands; ++l) {
				GOTEntityWesterlandsFarmhand farmhand = new GOTEntityWesterlandsFarmhand(world);
	...
	public static class Tree extends GOTStructureWesterlandsVillageFarm {
		...
		public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
			...
			for (int l = 0; l < 16; ++l) {
				...
				int j1 = 1;
				int k12 = 0;
				if (treeGen.generate(world, random, getX(i12, k12), getY(j1), getZ(i12, k12))) {
					break;
				}
			}
			for (i1 = -4; i1 <= 4; ++i1) {
