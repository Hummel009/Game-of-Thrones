	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		for (k12 = 0; k12 <= 19; ++k12) {
			...
			if (k12 == 8 || k12 == 11) {
				...
				setBlockAndMetadata(world, 6, 5, k12, brickBlock, brickMeta);
				setBlockAndMetadata(world, 6, 6, k12, brickSlabBlock, brickSlabMeta);
				placeWallBanner(world, -5, 3, k12, banner, 3);
				placeWallBanner(world, 5, 3, k12, banner, 1);
			}
			if (k12 != 9 && k12 != 10) {
				continue;
			}
		...
		for (i12 = -3; i12 <= 3; ++i12) {
			...
			if (Math.abs(i12) == 2) {
				placeWallBanner(world, i12, 4, 1, banner, 2);
			}
		...
		setBlockAndMetadata(world, 2, 5, 17, Blocks.torch, 4);
		placeWallBanner(world, 0, 5, 1, banner, 0);
		placeWallBanner(world, 0, 5, 18, banner, 2);
