...
public class GOTStructureArrynTower extends GOTStructureArrynBase {
	...
	public boolean generateAndHeight(World world, Random random, int i, int j, int k, int rotation, int height) {
		...
		if (generateRoom) {
			...
			setBlockAndMetadata(world, 1, doorBase + 1, -1, GOTRegistry.tableArryn, 0);
			placeWallBanner(world, 2, doorBase + 4, -1, GOTItemBanner.BannerType.ARRYN, 3);
			this.placeChest(world, random, -1, doorBase + 1, -1, GOTRegistry.chestStone, 3, GOTChestContents.WESTEROS);
		...
		respawner.setSpawnClass(GOTEntityArrynSoldierArcher.class);
