...
public class GOTStructureReachWatchtower extends GOTStructureReachBase {
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		setBlockAndMetadata(world, 3, 6, 0, GOTRegistry.brick1, 5);
		placeWallBanner(world, 0, 5, -3, GOTItemBanner.BannerType.TYRELL, 2);
		for (j1 = 1; j1 <= 9; j1++) {
		...
		placeBanner(world, 0, 14, 0, GOTItemBanner.BannerType.TYRELL, 2);
		...
		GOTEntityReachSoldier levyman = new GOTEntityReachSoldier(world);
		spawnNPCAndSetHome(levyman, world, 0, 11, 1, 16);
		GOTEntityNPCRespawner respawner = new GOTEntityNPCRespawner(world);
		respawner.setSpawnClass(GOTEntityReachSoldier.class);
