...
public class GOTStructureRuins extends GOTStructureBaseSettlement {
	...
	public static class GOTStructureRuinsPart extends GOTStructureBase {
		protected GOTStructureRuinsPart(boolean flag) {
			super(flag);
		}
	...
	public static class GOTStructureRuinsTower extends GOTStructureBase {
		protected GOTStructureRuinsTower(boolean flag) {
			super(flag);
		}
			...
			switch (rotation1) {
				case 0: {
					...
					for (i1 = i3 - 1; i1 <= i3 + 1; ++i1) {
						height = j4 + 1 + random.nextInt(3);
						for (j1 = j4; j1 <= height; ++j1) {
		...
		protected void placeIronBars(World world, Random random, int i, int j, int k) {
		...
		protected void placeRandomBrick(World world, int i, int j, int k) {
			setBlockAndNotifyAdequately(world, i, j, k, GOTBlocks.brick1, 1);
		}
	}
