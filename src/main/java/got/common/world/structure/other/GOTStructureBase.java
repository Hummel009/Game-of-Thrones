...
public abstract class GOTStructureBase extends WorldGenerator {
	...
	public boolean generateSubstructureWithRestrictionFlag(GOTStructureBase str, World world, Random random, int i, int j, int k, int r, boolean isRestrict) {
		...
		return str.generate(world, random, i, j, k, r %= 4);
	}
	...
	public boolean isSurface(World world, int i, int j, int k) {
		...
		i = getX(i1, k1);
		k = getZ(i1, k1);
		if (GOTStructureBase.isSurfaceStatic(world, i, j = getY(j), k)) {
			return true;
		}
	...
	public void placePlate_do(World world, Random random, int i, int j, int k, Block plateBlock, ItemStack foodItem, boolean certain) {
		...
		if ((certain || random.nextBoolean()) && (tileentity = getTileEntity(world, i, j, k)) instanceof GOTTileEntityPlate) {
	...
	public void placePlate_list(World world, Random random, int i, int j, int k, Block plateBlock, GOTFoods foodList, boolean certain) {
	...
	public static boolean isSurfaceStatic(World world, int i, int j, int k) {
		...
		if (block == Blocks.grass || block == Blocks.dirt || block == Blocks.gravel || block == GOTRegistry.dirtPath) {
			return true;
		}
