...
public class GOTStructureVolantisFortress extends GOTStructureVolantisBase {
	public GOTStructureVolantisFortress(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		this.placeBarrel(world, random, 3, 2, 4, 5, GOTFoods.ESSOS_DRINK);
		this.placeMug(world, random, 3, 2, 5, 1, GOTFoods.ESSOS_DRINK);
		this.placeChest(world, random, -1, 1, 8, GOTRegistry.chestBasket, 4, GOTChestContents.ESSOS);
		setBlockAndMetadata(world, -5, 1, 1, GOTRegistry.commandTable, 0);
		int warriors = 5 + random.nextInt(5);
		for (int l = 0; l < warriors; ++l) {
			GOTEntityVolantisLevyman warrior = new GOTEntityVolantisLevyman(world);
		...
		GOTEntityVolantisGeneral captain = new GOTEntityVolantisGeneral(world);
		captain.spawnRidingHorse = false;
		spawnNPCAndSetHome(captain, world, 0, 1, 4, 8);
		GOTEntityNPCRespawner respawner = new GOTEntityNPCRespawner(world);
		respawner.setSpawnClasses(GOTEntityVolantisLevyman.class, GOTEntityVolantisLevymanArcher.class);
