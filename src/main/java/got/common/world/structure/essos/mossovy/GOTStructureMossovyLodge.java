...
public class GOTStructureMossovyLodge extends GOTStructureMossovyBase {
	public GOTStructureMossovyLodge(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		this.placeChest(world, random, -4, 1, -1, 4, GOTChestContents.MOSSOVY);
		...
		for (int k131 : new int[] { -2, 2 }) {
			...
			if (random.nextInt(3) == 0) {
				armor = new ItemStack[] { new ItemStack(GOTRegistry.mossovyChestplate), new ItemStack(GOTRegistry.mossovyLeggings), new ItemStack(GOTRegistry.mossovyBoots) };
			}
		...
		for (int i1 : new int[] { -1, 1 }) {
			...
			this.placeChest(world, random, i1, -2, 3, 2, GOTChestContents.MOSSOVY, amount);
		}
