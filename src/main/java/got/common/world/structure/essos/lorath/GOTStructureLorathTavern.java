...
public class GOTStructureLorathTavern extends GOTStructureLorathBase {
	public GOTStructureLorathTavern(boolean flag) {
		super(flag);
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k, int rotation) {
		...
		placeWallBanner(world, 2, 5, -15, bannerType, 0);
		GOTEntityLorathBartender bartender = new GOTEntityLorathBartender(world);
		spawnNPCAndSetHome(bartender, world, -2, 1, 8, 4);
		int npc = 4 + random.nextInt(10);
		for (int l = 0; l < npc; ++l) {
			GOTEntityLorathMan npc1 = new GOTEntityLorathMan(world);
