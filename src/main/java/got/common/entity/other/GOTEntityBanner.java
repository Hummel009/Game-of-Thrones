		...
		for (int index = 0; index < maxSendIndex; ++index) {
			...
			if (profile instanceof GOTFellowshipProfile) {
				...
				if (StringUtils.isNullOrEmpty(username = profile.getName())) {
					whitelistSlots[index] = null;
					if (index == 0) {
						GOTLog.getLogger().info("GOT: Banner needs to be replaced at " + MathHelper.floor_double(posX) + " " + MathHelper.floor_double(posY) + " " + MathHelper.floor_double(posZ) + " dim_" + dimension);
					}
				} else {
