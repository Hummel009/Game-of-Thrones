...
public class GOTEntityNorvosMan extends GOTEntityHumanBase implements IPickpocketable {
	public GOTEntityNorvosMan(World world) {
		...
		tasks.addTask(2, createNorvosAttackAI());
	...
	public GOTMQ createMiniQuest() {
		return GOTMQFactory.NORVOS.createQuest(this);
	}
	...
	public void dropEssosItems(boolean flag, int i) {
		if (rand.nextInt(5) == 0) {
			dropChestContents(GOTChestContents.NORVOS, 1, 2 + i);
		}
	}
	...
	public float getBlockPathWeight(int i, int j, int k) {
		...
		if (biome instanceof GOTBiomeNorvos) {
			f += 20.0f;
		}
		return f;
	}
