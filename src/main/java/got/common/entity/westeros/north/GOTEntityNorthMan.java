...
public class GOTEntityNorthMan extends GOTEntityHumanBase implements IPickpocketable {
	...
	public GOTEntityNorthMan(World world) {
		...
		tasks.addTask(2, createNorthAttackAI());
	...
	public void dropWesterosItems(boolean flag, int i) {
		if (rand.nextInt(6) == 0) {
			dropChestContents(GOTChestContents.NORTH_HOUSE, 1, 2 + i);
		}
	}
	...
	public float getBlockPathWeight(int i, int j, int k) {
		...
		if (biome instanceof GOTBiomeNorth) {
			f += 20.0f;
		}
		return f;
	}
