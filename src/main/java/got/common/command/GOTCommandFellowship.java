...
public class GOTCommandFellowship extends CommandBase {
	...
	public List<String> addTabCompletionOptions(ICommandSender sender, String[] args) {
		String[] args1 = args;
		if (args1.length == 1) {
			return CommandBase.getListOfStringsMatchingLastWord(args1, "create", "option");
		}
		if (args1.length == 2) {
			return CommandBase.getListOfStringsMatchingLastWord(args1, MinecraftServer.getServer().getAllUsernames());
		}
		if (args1.length > 2) {
			...
			if ("option".equals(function)) {
				...
				if (ownerID != null) {
					...
					if (fsName != null && (fellowship = playerData.getFellowshipByName(fsName)) != null) {
						if (args1.length == 4) {
							return CommandBase.getListOfStringsMatchingLastWord(args1, "invite", "add", "remove", "transfer", "op", "deop", "disband", "rename", "icon", "pvp", "hired-ff", "map-show");
						}
						...
						if ("invite".equals(option) || "add".equals(option)) {
							...
							return CommandBase.getListOfStringsMatchingLastWord(args1, notInFellowshipNames.toArray(new String[0]));
						}
						if ("remove".equals(option) || "transfer".equals(option)) {
							...
							return CommandBase.getListOfStringsMatchingLastWord(args1, memberNames.toArray(new String[0]));
						}
						if ("op".equals(option)) {
							...
							return CommandBase.getListOfStringsMatchingLastWord(args1, notAdminNames.toArray(new String[0]));
						}
						if ("deop".equals(option)) {
							...
							return CommandBase.getListOfStringsMatchingLastWord(args1, adminNames.toArray(new String[0]));
						}
						if ("pvp".equals(option) || "hired-ff".equals(option)) {
							return CommandBase.getListOfStringsMatchingLastWord(args1, "prevent", "allow");
						}
						if ("map-show".equals(option)) {
							return CommandBase.getListOfStringsMatchingLastWord(args1, "on", "off");
						}
					}
				}
			}
