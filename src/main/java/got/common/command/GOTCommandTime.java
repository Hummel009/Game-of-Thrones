	...
	public void processCommand(ICommandSender sender, String[] args) {
		if (args.length >= 2) {
			if ("set".equals(args[0])) {
				long time;
				time = "day".equals(args[1]) ? Math.round(GOTTime.getDayLength() * 0.03) : "night".equals(args[1]) ? Math.round(GOTTime.getDayLength() * 0.6) : (long) CommandBase.parseIntWithMin(sender, args[1], 0);
